...
const commands = {
	'play': (msg) => {
		...
		(function play(song) {
			...
			msg.channel.sendMessage(`Çalýnan: **${song.title}** talep eden: **${song.requester}**`);
			...
			collector.on('message', m => {
				if (m.content.startsWith(ayarlar.prefix + 'pause')) {
					...
					msg.channel.sendMessage(`Þiddet: ${Math.round(dispatcher.volume*50)}%`);
				} else if (m.content.startsWith('volume-')){
	...
	'add': async (msg) => {
		...
		try {
			...
			try {
				...
				console.log(err)
				message.channel.send('Bir hata oluþtu: ' + err)
			};
		};
		...
		yt.getInfo(url, (err, info) => {
			if(err) return msg.channel.sendMessage('Geçersiz YouTube Baðlantýsý: ' + err);
			if (!queue.hasOwnProperty(msg.guild.id)) queue[msg.guild.id] = {}, queue[msg.guild.id].playing = false, queue[msg.guild.id].songs = [];
			queue[msg.guild.id].songs.push({url: url, title: info.title, requester: msg.author.username});
			msg.channel.sendMessage(`sýraya **${info.title}** eklendi`);
		});
	},
	'queue': (msg) => {
		...
		msg.channel.sendMessage(`__**${msg.guild.name}'s Müzik Kuyruðu:**__ Þu anda **${tosend.length}** þarký sýrada ${(tosend.length > 15 ? '*[Sadece 15 tanesi gösteriliyor]*' : '')}\n\`\`\`${tosend.slice(0,15).join('\n')}\`\`\``);
	}
};

client.on('ready', () => {
	console.log('BOT: Bot hazýr.');
});
